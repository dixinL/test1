...
def main():
    ...
    print("   参数: no_image={}, no_report={}".format(no_image, no_report))
    sys.stdout.flush()
    ...
    results["rss_html"] = ok
    step_elapsed = (datetime.now() - step_start).total_seconds()
    print("   [Step 4] 生成 RSS HTML: {} (用时: {:.1f}s)".format(
        "成功" if ok else "失败",
        step_elapsed
    ))
    sys.stdout.flush()
